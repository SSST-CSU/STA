                ...
                <caption>
                    <center>
                        <div class="input-group" style="width: 50vw;">
                            <input id="searchKey" type="text" class="form-control" placeholder="用输入要查找的比赛的关键字" value="${sessionScope.key}"
            ...
            <nav>
                <ul class="pagination clean">
                    ...
                    <c:forEach begin="${sessionScope.startAndEnd.start}" end="${sessionScope.startAndEnd.end}"
                        ...
                        <a href="adminCompetition?targetPage=${status.index}&key=${sessionScope.key}">${status.index}</a>
                        </li>
                    </c:forEach>
                    <li>
...
<script>
    ...
    function previousPage() {
        if (${sessionScope.targetPage<=1}) {
            window.location.href = "adminCompetition?targetPage=1&key=${sessionScope.key}";
        }
        else {
            window.location.href = "adminCompetition?targetPage=${sessionScope.targetPage-1}&key=${sessionScope.key}";
        }
    }
    function nextPage() {
        if (${sessionScope.targetPage>=sessionScope.pageNumber}) {
            window.location.href = "adminCompetition?targetPage=${sessionScope.pageNumber}&key=${sessionScope.key}";
        }
        else {
            window.location.href = "adminCompetition?targetPage=${sessionScope.targetPage+1}&key=${sessionScope.key}";
        }
