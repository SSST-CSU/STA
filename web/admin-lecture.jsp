                ...
                <ul class="pagination clean">
                    ...
                    <c:forEach begin="${requestScope.startAndEnd.start}" end="${requestScope.startAndEnd.end}"
                        ...
                        <li<c:if test="${status.index==requestScope.targetPage}"> class="active"</c:if>>
                            <a href="adminLecture?targetPage=${status.index}&key=${requestScope.key}">${status.index}</a>
                        </li>
                    </c:forEach>
                    <li>
...
<script>
    ...
    function previousPage() {
        if (${requestScope.targetPage<=1}) {
            window.location.href = "adminLecture?targetPage=1&key=${requestScope.key}";
        }
        else {
            window.location.href = "adminLecture?targetPage=${requestScope.targetPage-1}&key=${requestScope.key}";
        }
    }
    function nextPage() {
        if (${requestScope.targetPage>=requestScope.pageNumber}) {
            window.location.href = "adminLecture?targetPage=${requestScope.pageNumber}&key=${requestScope.key}";
        }
        else {
            window.location.href = "adminLecture?targetPage=${requestScope.targetPage+1}&key=${requestScope.key}";
        }
